    ...
    for word in words:
        ...
        for i in range(len(word_list)):
            ...
            if word_list[i] == word:
                ...
                # Record the index change for summary
                if word not in index_changes:
                    initial_index = i
                    # Adjust the final index by subtracting 1
                    final_index = i - 1 if i > 0 else i
                    index_changes[word] = f"{initial_index} -> {final_index}"
        ...
        if not found:
            ...
            # Record index change for new word
            # Adjust the final index by subtracting 1
            final_index = len(word_list) - 1
            adjusted_final_index = final_index - 1
            index_changes[word] = f"None -> {adjusted_final_index}"
